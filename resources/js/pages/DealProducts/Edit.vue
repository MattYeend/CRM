...
const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { title: `Edit ${props.product.name}`, href: route('deals.products.edit', { deal: props.deal.id, product: props.product.id }) },
]
...
async function submit() {
    ...
    try {
        await updateDealProducts(props.deal.id, {
            products: [
                {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        <Head :title="`Edit Product — ${deal.title}`" />
        <div class="p-6">
            ...
            <p class="text-gray-500 mb-6">Updating product on deal: <span class="font-medium text-gray-800">{{ deal.title }}</span></p>
