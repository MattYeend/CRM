...
import { fetchPipelines, deletePipelines } from '@/services/pipelineService'
...
async function handleDelete(id: number) {
    ...
    deletingId.value = id
    try {
        await deletePipelines(id)
        loadPipelines(currentPage.value)
    } finally {
            ...
            <table v-else class="w-full border">
                <thead>
                    <tr>
                        ...
                        <th class="p-2 text-left">Stages</th>
                ...
                <tbody>
                    <tr v-for="pipeline in items" :key="pipeline.id" class="border-t">
                        ...
                        <td class="p-2">{{ pipeline.description || '—' }}</td>
                        ...
                        <td class="p-2">
                            <span
                                :class="pipeline.is_default
                                    ? 'bg-blue-100 text-blue-800'
                                    : 'bg-gray-100 text-gray-800'"
                                class="px-2 py-1 rounded text-xs"
                            >
                                {{ pipeline.is_default ? 'Default' : 'Active' }}
                            </span>
                        </td>

                        <td class="p-2 space-x-2 text-right">
                            <Link
                                v-if="pipeline.permissions.view"
                                :href="route('pipelines.show', pipeline.id)"
                            >
                                View
                            </Link>
                            ...
                            <Link
                                ...
                                :href="route('pipelines.edit', pipeline.id)"
                            >
                                Edit
                            </Link>
